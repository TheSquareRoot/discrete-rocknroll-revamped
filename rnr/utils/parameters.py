...
def check_config(config: dict) -> None:
    ...
    if config["adhdistrib"]["dist_params"] == "biasi":
        if (config["adhdistrib"]["medians"] is not None) or (
            config["distribution"]["spreads"] is not None
        ):
            logger.warning(
                "Biasi parameters selected. Custom median and scatter parameters will be ignored."
            )
    ...
    if config["simulation"]["duration"] < config["simulation"]["dt"]:
        logger.error(
            f"dt must be smaller than total duration. dt set to {config['simulation']['duration']:.2f}s"
        )
